...
\begin{enumerate}
  ...
  \item \textbf{The three-path framework carries non-redundant information.} Section~\ref{sec:results_contamination} reports the cell~$\times$~path interaction ANOVA (Eq.~\ref{eq:anova_interact}) under Type-III sums of squares: $F(36, 4867) = 6.47$, $p < 0.001$, partial $\eta^2 = 0.046$. The mixed-effects logit (Section~\ref{sec:results_mixed_logit}, Eq.~\ref{eq:mixedlogit}) qualitatively agrees: test-stage substitutions carry significant per-stage attribution while spec- and code-stage substitutions are null at $\alpha = 0.05$. Cells respond \emph{differently} to different closure paths, not with a constant offset. The three-path framework of Section~\ref{sec:framework} is empirically justified rather than merely stipulated.
