...
\begin{enumerate}
  \item \textbf{Pre-registered hypothesis validation at per-operator resolution.} The H1 cell (phi-4 as $L_{\textsf{spec}}$, qwen-coder as $L_{\textsf{test}}$ and $L_{\textsf{code}}$) was assigned in the pre-registered DOE based on the pre-registered per-mutation-operator capability decomposition. Section~\ref{sec:results_per_operator} shows that H1 dominates four of five mutation operators empirically, with the largest gain (arithmetic, $\Delta = +0.133$) matching the pre-registered prediction that phi-4 wins on predicate reasoning. Pre-registered stage compositions can be empirically validated by data at the operator level.
