...
First, the qualitative RQ1 finding of Section~\ref{sec:discussion_rq1} --- that heterogeneity matches mono on aggregate metric mean $\bar{Y}$ and dominates by operator or by strict-AND $V$-scale on selected cells --- holds at every $(\tau, \rho) \in \{0, 0.5\} \times \{2, 3\}$. On the conditional-on-metric $V'$ scale of Table~\ref{tab:threshold_sensitivity_path1}, the top-ranked cell at the reference $(\tau=0, \rho=3)$ is H9 ($V' = 0.885$), with H11 ($0.859$), H6 ($0.855$), M3 ($0.855$), and H1 ($0.790$) clustered within $\sim$$0.10$; the ordering shuffles slightly across threshold pairs but no mono cell dominates all hetero cells at any $(\tau, \rho) \in \{0, 0.5\} \times \{2, 3\}$. At the extreme $\rho = 4$ (``identical only''), the entire distribution compresses toward zero and threshold-choice noise dominates. Kendall's $\tau_{\textsf{rank}}$ between the reference threshold and every alternative $(\tau, \rho) \in \{0, 0.5, 0.8\} \times \{2, 3\}$ exceeds $0.65$, indicating that the ordinal ranking of cells is preserved as thresholds are varied within the paper's operational band.
