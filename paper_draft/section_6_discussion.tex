...
\textbf{Threat.} The core dataset comprises 150~functions from HumanEval and MBPP. Both benchmarks are Python-only, function-level (not multi-file or class-level), and drawn from a corpus that appeared in the training data of many of the pipeline SLMs. Contamination in closed and open LLM evaluation is a recognised systemic problem~\citep{Balloccu2025LLMDataContamination}. The results may not transfer to other benchmarks, other languages, or to multi-file / class-level SE tasks.
