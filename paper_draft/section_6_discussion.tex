...
Second, the stage-attribution decomposition (Section~\ref{sec:results_stage_attribution}), evaluated on the strict-AND valid-closure fraction $V$ (Eq.~\ref{eq:valid_closure_frac}), shows that H4 and H11 exceed the strongest mono ceiling on Path~1 outright ($V_{H4,1} = 0.909$ and $V_{H11,1} = 0.859$ vs.\ $V_{M4,1} = 0.773$). This is a $V$-scale claim; on the metric-mean $\bar{Y}$ scale, M3 still leads. Both hetero cells specialize at the code and test stages while permitting a weaker model at spec (H4 uses llama3.2 at code, H11 uses phi-4 at spec) --- validating that the heterogeneity benefit compounds when specialization matches the demanded stage strength.
...
\textbf{RQ4 answer.} The mean false-closure rate across cells is $7.7\%$; the maximum non-null-cell rate is $28.6\%$ (H10 on Path~1); the null cell N1 (word-shuffled input) exhibits a rate of $26.3\%$, which serves as an empirical noise floor.
...
The false-closure result is therefore not a limitation but a \emph{characterization} of the metric's operating regime. Section~\ref{sec:experiment_setup_sensitivity} shows that alternative threshold choices do not meaningfully change these figures.
...
First, the qualitative RQ1 finding of Section~\ref{sec:discussion_rq1} --- that heterogeneity matches but does not universally dominate the strongest mono baseline on aggregate closure rate, and dominates when decomposed by operator --- holds at every $(\tau, \rho) \in \{0, 0.5\} \times \{2, 3\}$. The strongest mono cell M3 remains the top-ranked cell on Path~1 at every threshold in this band ($0.900$ at the reference $(\tau=0, \rho=3)$), and the best hetero cells (H1, H6, H9, H11) remain within 0.05 of M3 at every threshold. At the extreme $\rho = 4$ (``identical only''), the entire distribution compresses toward zero and threshold-choice noise dominates. Kendall's $\tau_{\textsf{rank}}$ between the reference threshold and every alternative $(\tau, \rho) \in \{0, 0.5, 0.8\} \times \{2, 3\}$ exceeds $0.65$, indicating that the ordinal ranking of cells is preserved as thresholds are varied within the paper's operational band.
