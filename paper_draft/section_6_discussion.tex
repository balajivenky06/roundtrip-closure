...
\textbf{Threat.} The five-layer checkpointing system (LLM cache, pytest cache, BERTScore cache, TSV row cache, JSONL decontamination cache) means that identical calls across cells are served from cache. This introduces a specific analytical concern: each closure path exercises only two of the three pipeline stages (Path~1 uses $L_{\textsf{spec}}$ and $L_{\textsf{test}}$; Path~2 uses $L_{\textsf{test}}$ and $L_{\textsf{code}}$; Path~3 uses $L_{\textsf{test}}$ and $L_{\textsf{spec}}$), so cells sharing the operative stage-pair on a given path produce byte-identical rows on that path. The per-path duplicate groups are:

\begin{itemize}
  \item \textbf{Path~1 (shared $L_{\textsf{spec}}, L_{\textsf{test}}$):} $\{M6, H4\}$, $\{H7, H9\}$, $\{M1, N1\}$ produce identical Path~1 rows within each group.
  \item \textbf{Path~2 (shared $L_{\textsf{test}}, L_{\textsf{code}}$):} $\{M6, H1, H5\}$, $\{H2, H10\}$, $\{H6, H7\}$, $\{M1, N1\}$ produce identical Path~2 rows within each group.
  \item \textbf{Path~3 (shared $L_{\textsf{test}}, L_{\textsf{spec}}$):} $\{M6, H4\}$, $\{H7, H9\}$, $\{M1, N1\}$ produce identical Path~3 rows within each group.
\end{itemize}

\textbf{Mitigation and correction.} We disclose these groups explicitly because Tukey pairwise contrasts and per-path effect sizes double-count rows that share an operative stage-pair, and between-duplicate contrasts (e.g.\ H7 vs.\ H9 on Path~1) are structurally null rather than empirically informative. In interpreting the per-path ANOVAs (Section~\ref{sec:results_stage_attribution}) and the mixed-effects logit (Section~\ref{sec:results_mixed_logit}), the effective degrees of freedom on any given path are reduced by the number of duplicate rows within groups; the qualitative RQ1--RQ3 conclusions (heterogeneity matches mono on aggregate $\bar{Y}$; test-stage is the significant per-stage attribution; the phi-4-in-test-stage pathology) do not depend on between-duplicate contrasts and survive the correction. The mixed-effects logit is the least affected because it estimates per-model-per-stage marginal effects rather than per-cell effects, so duplicates on a given path collapse to the same fitted probability. Table~\ref{tab:mixed_logit}'s significant test-stage coefficients ($\beta \neq 0$ for phi-4, mistral-small, llama3.2 at $L_{\textsf{test}}$) therefore remain the paper's most duplication-robust per-stage finding.

\textbf{Residual risk.} Tukey contrasts between duplicate-group members overstate the effective per-comparison sample size; between-group contrasts are unaffected. Where duplicate-group members appear in headline contrasts (M6-M1, H9-H7, H1-M6), we mark them non-informative in the notes below Table~\ref{tab:tukey_significant}.
...
\textbf{Residual risk.} Findings are scoped to open-weight SLMs under 30~B parameters. Reviewers may reasonably ask ``do these findings apply to closed-weight frontier models?'' --- the honest answer is that this study cannot say. The follow-up outlined in Section~7 addresses this directly.
