...
%% (× → İ, ~ → í, ∅ → ç). Fix: swap in Times-family math which has correct
%% slots. cas-sc conflicts with newtxmath but not with mathptmx (older,
%% widely-compatible Times math package).
...
first cross-LLM by cross-stage evaluation of round-trip closure
...
produces filter-failing tests on approximately 50\% of samples across
three cells---invisible in single-stage evaluations. Third, BERTScore
F1 is uncorrelated in aggregate (Pearson r = 0.02, n = 1{,}791) with
the judge on the docstring path; disagreement flips across benchmarks,
consistent with a surface-form signal. Fourth, the three closure paths
carry non-redundant information about cell performance (interaction
F-statistic 6.21 on 34 and 4{,}865 degrees of freedom, p less than
0.001, partial eta-squared 0.042). Fifth, heterogeneity is not free:
well-composed hetero cells preserve 78--91\% of the strongest mono
baseline, while poorly-composed cells collapse. A HumanEval-Mutated
decontamination re-sweep shows behavioral-path closure rates hold or
improve under function-rename plus docstring-paraphrase, ruling out
surface-form memorization. We derive three design guidelines and
...
%% ============================================================================
\begin{highlights}
\item First cross-LLM by cross-stage study of round-trip closure in SE pipelines
