...
%% -- Fonts + microtype ------------------------------------------------------
%% cas-sc's default math font ships with broken glyph mappings for several
%% common symbols (× → İ, % → ˜, ∅ → ç, ' → odd chars). Fix:
%%   1. T1 fontenc + utf8 inputenc (character-encoding sanity)
%%   2. lmodern text font (avoids the broken cas-sc default text font)
%%   3. textcomp for text-mode symbols
%%   4. newtxmath for a Times-based math font that has correct × / ∈ / % glyphs
%%   5. mathptmx as a further fallback if newtxmath is unavailable
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{textcomp}
\usepackage{newtxmath}      % swaps cas-sc's default math font → Times-based math
...
%% Title + tnotes
%% ============================================================================
\title[mode=title]{Heterogeneous Multi-SLM Closure of the Docstring--Test--Code Triangle: A Mutation-Testing Study}
...
a general LLM-as-judge property. Five findings emerge. First, the H1
composition (phi-4 spec + qwen-coder test/code) dominates four of five
mutation operators as pre-registered. Second, phi-4 in the test stage
produces filter-failing tests on ${\sim}50\%$ of samples across three
cells---invisible in single-stage evaluations. Third, BERTScore F1 is
uncorrelated in aggregate ($r = 0.02$, $n = 1{,}791$) with the judge
on the docstring path; disagreement flips across benchmarks, consistent
with a surface-form signal. Fourth, the three closure paths carry
non-redundant information about cell performance
($F(34, 4865) = 6.21$, $p < 0.001$, partial $\eta^2 = 0.042$). Fifth,
heterogeneity is not free: well-composed hetero cells preserve
$78$--$91\%$ of the strongest mono baseline, while poorly-composed
cells collapse. A HumanEval-Mutated decontamination re-sweep shows
behavioral-path closure rates hold or improve under function-rename +
docstring-paraphrase, ruling out surface-form memorization. We derive
three design guidelines and release an open replication package with
67 unit tests.
\end{abstract}
