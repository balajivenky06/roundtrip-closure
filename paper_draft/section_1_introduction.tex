...
A sixth methodological contribution is the demonstration that open-weight SLM evaluation with pre-registered statistical rigor and transparent computational-cost accounting is a viable methodology for empirical software-engineering research. The six pipeline SLMs used here are all under 30~B parameters and all released with open weights; the entire 5{,}890-row sweep ran on a single-researcher Colab~Pro+ subscription ($\sim$530 GPU-hours across A100 and T4, an estimated $25$~kg~CO$_2$-equivalent following~\citet{Lacoste2019MLCO2Emissions} --- see Ethics statement).
