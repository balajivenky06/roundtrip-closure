...
\begin{enumerate}
  ...
  \item \textbf{Empirical per-operator dominance of the H1 composition.} H1 (phi-4 at $L_{\textsf{spec}}$, qwen-coder at $L_{\textsf{test}}$ and $L_{\textsf{code}}$) empirically achieves the highest kill rate on 4 of 5 mutation-operator families (arithmetic $\Delta = +0.133$ vs.\ best mono; boundary $+0.096$; comparison $+0.088$; negate\_bool $+0.056$; return\_none is won by H6 at $\Delta = +0.010$). The per-operator pattern is consistent with capability priors from the mutation-testing literature~\citep{Andrews2005MutationAnalysis, Just2014MutationAnalysis}, in which arithmetic and comparison operators are known to be the discriminating operators for test-suite adequacy. To our knowledge this is the first cross-stage evaluation of an SLM composition at operator resolution.
