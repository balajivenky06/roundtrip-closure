...
Table~\ref{tab:closure_rate_matrix} and Fig.~\ref{fig:closure_heatmap} present the per-cell metric mean $\bar{Y}_{a, p}$ (Eq.~\ref{eq:metric_mean}) for every cell $\times$ path combination. This subsection reports metric means; the strict-AND valid-closure fraction $V_{a, p}$ is reported per-cell in Sections~\ref{sec:results_stage_attribution}, \ref{sec:results_contamination}, and~\ref{sec:human_eval_strict_and}. Three high-level patterns emerge.
...
This subsection reports strict-AND valid-closure fractions $V_{a, p}$ (Eq.~\ref{eq:valid_closure_frac}); numeric values here therefore differ from Table~\ref{tab:closure_rate_matrix}'s metric means $\bar{Y}_{a, p}$. Each hetero cell composes three models $(m_{\textsf{spec}}, m_{\textsf{test}}, m_{\textsf{code}})$.

\input{../tables/tab_per_stage_bottleneck.tex}

Table~\ref{tab:per_stage_bottleneck} attributes each hetero cell's valid-closure fraction to the mono cell whose model appears at the same slot: for hetero cell $a$ and stage $s$, the \emph{stage baseline} $\hat{V}_{a,s}$ is the valid-closure fraction of the mono cell using $a(s)$ at all stages. The \emph{bottleneck stage} is the stage whose baseline is the lowest, and $\Delta_a = V_a - \min_s \hat{V}_{a,s}$ is the improvement from specializing the other two stages.
...
\subsection{Mixed-effects logit: per-stage attribution with sample random intercept}
\label{sec:results_mixed_logit}

To confirm the per-stage attribution of Section~\ref{sec:results_stage_attribution} in a single unified model, we fit the mixed-effects specification of Eq.~\ref{eq:mixedlogit} on strict-AND valid closure ($V_r \in \{0, 1\}$; Eq.~\ref{eq:valid_closure_frac} at the row level) as the outcome, with the three per-stage model assignments as fixed effects and a per-sample random intercept. Table~\ref{tab:mixed_logit} presents the fitted coefficients, using qwen3.6 (the mid-range dense mono baseline M3) as the reference category at each stage.

\input{../tables/tab_mixed_logit.tex}

Three findings emerge that support and refine the per-stage attribution reported in Section~\ref{sec:results_stage_attribution}:

\begin{enumerate}
  \item \textbf{Test-stage substitutions are the significant per-stage effects.} Relative to qwen3.6 at $L_{\textsf{test}}$, placing llama3.2 at the test stage lowers $\Pr[V=1]$ by $27.1$~pp ($p < 0.001$), mistral-small at the test stage by $12.6$~pp ($p = 0.001$), and phi-4 at the test stage by $10.0$~pp ($p = 0.002$). All other test-stage substitutions and every spec- and code-stage substitution are not significant at $\alpha = 0.05$.
  \item \textbf{The phi-4-in-test-stage pathology is a mixed-model coefficient, not just a NA-rate observation.} Section~\ref{sec:discussion_rq3}'s Path~1 \textsf{structural\_NA} rates ($44$--$55\%$ across M2/H2/H10) and the mixed-logit test-stage coefficient for phi-4 ($-0.100$, $p = 0.002$) are two orthogonal statistical instruments converging on the same finding. This is the strongest single-cell-independent confirmation of the phi-4-in-test-stage claim.
  \item \textbf{Path is a strong main effect after controlling for stage assignment.} Path~2 vs.\ Path~1: $-25.5$~pp ($p < 0.001$); Path~3 vs.\ Path~1: $-18.2$~pp ($p < 0.001$). This corroborates the interaction-ANOVA finding (Section~\ref{sec:results_stage_attribution}) that the paths carry non-redundant information: differences persist after cell-effect controls.
\end{enumerate}

The mixed-logit converges to a linear-probability approximation rather than the full Bernoulli GLMM at this data volume ($n = 4{,}996$ rows, 150 sample groups). Random-intercept variance is $\sigma_{\textsf{sample}}^2 = 0.053$, indicating meaningful per-sample difficulty heterogeneity above what the fixed effects capture. Full ANOVA and mixed-logit conclusions agree qualitatively on the pattern that cell composition drives significant variation in $V$ with the test stage carrying the plurality of the attribution.
...
To test whether the three closure paths carry non-redundant information, we fit the interaction ANOVA of Eq.~\ref{eq:anova_interact}. Table~\ref{tab:anova_interaction} presents the results for $n = 5{,}073$ rows after excluding structural NaN entries and cells present only on a subset of paths.

The cell $\times$ path interaction is significant under Type-III sums of squares ($F(36, 4867) = 6.47$, $p < 0.001$, partial $\eta^2 = 0.046$). The effect is statistically robust given the large sample ($n = 5{,}073$) but small in Cohen's conventional bands ($\eta^2_p \in [0.01, 0.06]$~=~small; $[0.06, 0.14]$~=~medium; $\geq 0.14$~=~large). Path assignment therefore accounts for a modest share of variance in closure rate once cell and sample are controlled. \emph{Caveat on the stacked ANOVA response.} Because the response variable stacks three different-scale metrics (kill rate $\in [0, 1]$, binary pass, rescaled BERTScore $\in (-\infty, 1]$), the path main effect in Table~\ref{tab:anova_interaction} is partly a response-scale artifact. To confirm the interaction is not a scale artifact, we re-fit \emph{per-path} rank-transformed ANOVAs (three separate models, cell + sample fixed effects, rank-transformed metric as response). The cell main effect is significant on every path with even stronger effect sizes than the stacked model: Path~1 $F = 22.31$, partial $\eta^2 = 0.229$; Path~2 $F = 16.38$, partial $\eta^2 = 0.150$; Path~3 $F = 15.73$, partial $\eta^2 = 0.141$. Cell-composition differences drive substantial variance in the metric on every path independently of the stacking scale.

The variance-explained metric alone understates the \emph{practical} implication of the interaction: cell rankings are unstable across paths, so a practitioner choosing a cell for one path cannot re-use the choice for another. We quantify this with a rank-swap analysis over the 17 non-null cells on the strict-AND valid-closure fraction $V_{a, p}$ (Eq.~\ref{eq:valid_closure_frac}):

\begin{itemize}
  \item \textbf{The best cell is different for every path.} Path~1 best = H4 ($V = 0.909$); Path~2 best = H3 ($V = 0.646$); Path~3 best = H9 ($V = 0.733$). No cell tops any two paths.
...
Under Type-III sums of squares the four ANOVA effects are (Table~\ref{tab:anova_interaction}, all $p < 0.001$): sample difficulty $\eta^2_p = 0.234$ (the substantial random-intercept-equivalent term), interaction $\eta^2_p = 0.046$, cell main effect $\eta^2_p = 0.044$, path main effect $\eta^2_p = 0.042$. The four effects are of comparable size once Type-III controls each for the others --- there is no single dominant effect, and the practical interpretation rests on the interaction, which is the smallest-per-df but most theoretically load-bearing term.
...
Section~\ref{sec:discussion} interprets these findings against RQ1--RQ4,
