...
\subsubsection{Judge--human agreement (pending addendum)}
...
where $\delta$ is the ordinal-difference metric and $D_e$ is the expected disagreement under random pairing. Values $\alpha > 0.667$ are considered acceptable for inferential use. Both $\kappa$ and $\alpha$ will be reported in the pending human-evaluation addendum described in Section~7.3.
