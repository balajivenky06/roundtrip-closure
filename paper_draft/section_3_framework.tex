...
\input{figures/fig_framework.tex}
...
$\bar{Y}_{a, p}$ (\emph{metric mean}) is the raw automated metric averaged across a cell's rows and appears in Table~\ref{tab:closure_rate_matrix}, Fig.~\ref{fig:closure_heatmap}, the RQ1 ANOVA of Section~\ref{sec:results_mono_vs_hetero}, and all Tukey pairwise contrasts. $V_{a, p}$ (\emph{valid-closure fraction}) is the strict-AND two-signal closure count of Algorithm~\ref{alg:validity} normalized by row count, and appears in Table~\ref{tab:per_stage_bottleneck}, the stage-contribution figure (Fig.~\ref{fig:stage_contribution}), the rank-swap analysis of Section~\ref{sec:results_contamination}, the capability-preservation table (Table~\ref{tab:capability_preservation}), the strict-AND ablation of Section~\ref{sec:human_eval_strict_and}, and the threshold-sensitivity table (Table~\ref{tab:threshold_sensitivity_path1}). Every downstream number that reads as ``closure rate'' is one of these two aggregates; captions and text below specify which.
