...
The choice of DeepSeek-R1:14B as judge is unusual --- prior LLM-as-judge protocols (Zheng et al.\ 2023 and successors) predominantly use frontier judges (GPT-4-class or Claude Sonnet/Opus-class) on the argument that judge quality correlates with model capability. Our choice is motivated by open-weight reproducibility: the entire evaluation stack can be re-run without proprietary API access. Whether the low $\kappa_{\textsf{quad}} = 0.074$ against humans reflects a property specific to DeepSeek-R1:14B or a more general LLM-as-judge limitation on this task cannot be established from this study alone. A targeted follow-up study replaying the same 60-pair sample under a frontier judge (GPT-4o-mini or Claude Haiku) would distinguish these two hypotheses at low cost (${\sim}60$ API calls); we identify this as an immediate next step.
...
\begin{itemize}
  ...
  \item \textbf{Human evaluation reported in addendum.} The 60-pair five-annotator study is reported in Section~\ref{sec:human_eval_addendum}, which post-dates the initial revision.
\end{itemize}
