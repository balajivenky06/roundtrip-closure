...
\textbf{Residual risk.} The judge's category-exact reliability against humans is weak ($\kappa_{\textsf{quad}} = 0.138$); the strict-AND policy that pairs judge and metric on every closure decision is the mitigation. Claims in this paper that depend on the judge's rating (Section~5.2, Section~5.4) are stated as ``the judge disagrees with the metric'' or ``the judge accepts equivalence'' rather than as ``the artefacts are semantically equivalent''.

The choice of DeepSeek-R1:14B as judge is unusual --- prior LLM-as-judge protocols (Zheng et al.\ 2023 and successors) predominantly use frontier judges (GPT-4-class or Claude Sonnet/Opus-class) on the argument that judge quality correlates with model capability. Our choice is motivated by open-weight reproducibility: the entire evaluation stack can be re-run without proprietary API access. Whether the low $\kappa_{\textsf{quad}} = 0.138$ against humans reflects a property specific to DeepSeek-R1:14B or a more general LLM-as-judge limitation on this task cannot be established from this study alone. A targeted follow-up study replaying the same 60-pair sample under a frontier judge (GPT-4o-mini or Claude Haiku) would distinguish these two hypotheses at low cost (${\sim}60$ API calls); we identify this as an immediate next step.
